...
let type_check_def tenv env (var,expr,typ_annot) =
  ...
  let type_additionnal env (v, nf) =
    let typ = System.CheckerNew.typeof_simple tenv env nf |> generalize_all in
    Env.add v typ env
  in
  try
    Utils.log "%a@." Msc.pp_e nf_expr ;
    let env = List.fold_left type_additionnal env nf_addition in
    let typ = System.CheckerNew.typeof_simple tenv env nf_expr |> generalize_all in
    let typ =
  ...
  with
  | System.CheckerNew.Untypeable (pos, str) ->
