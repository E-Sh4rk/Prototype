...
let strengthen v t tt =
  ...
  | Ref (b, r) ->
    if mem v b && subtype (find v b) t then Ref (b, r)
    else Ref (b, Env.strengthen v t r)
let refine_old v t tt =
  try
    let ot = find v tt in
    if (Typ.is_empty ot |> not) && disjoint t ot then None
    else Some (strengthen v t tt)
  with Not_found -> None
let refine v t tt =
  let ot = find v tt in
  if (subtype any_or_absent ot |> not) &&
      (Typ.is_empty ot |> not) && disjoint t ot
  then None
  else Some (strengthen v t tt)
let rm_ref v t =
  match t with
...
let neg_refs b ts =
  let merge t1 t2 =
    ...
    | (x,t)::lst ->
      ...
      if Typ.is_empty (find x acc) && Typ.is_empty t |> not
