...
let splits_domain splits domain =
  Format.asprintf "Splits: %a - Domain: %a"
    pp_splits splits pp_typ domain
...
let sufficient_a env mono a prev_t t =
  ...
  | App (v1, v2) ->
    ...
    Types.Tvar.Raw.tallying_infer (poly_vars |> TVarSet.destruct) mono [(lhs,rhs)] |>
    List.filter_map (fun s ->
      let s = Subst.find s tmpvar in
      let s = Types.Tvar.Raw.clean_type ~pos:any ~neg:empty mono s in
...
let rec infer_a' ?(no_lambda_ua=false) pos tenv env mono anns a ts =
  ...
  let type_app t1 t2 t =
    ...
    let substs = Types.Tvar.Raw.tallying_infer (poly_vars |> TVarSet.destruct) vars_t [(lhs, rhs)] in
    let res =
  ...
  then begin (* Option *)
    ...
    | Lambda (_, Ast.ADomain ss, v, e), PLambdaA (former_typ, va) ->
      let s = disj ss in
    ...
    let res =
      ...
      | splits ->
        ...
        | Some anns_a ->
          ...
          log "@,Type of the definition: %a" pp_typ s ;
